// purchaseStore.js - Performance optimized version with lifecycle methods
...
const perfMonitor = {
  start: (operation) => console.time(`⚡ ${operation}`),
  end: (operation) => console.timeEnd(`⚡ ${operation}`)
};
...
export const usePurchaseStore = create((set, get) => ({
  ...
  initializePurchasePage: async () => {
    perfMonitor.start('initializePurchasePage');
    ...
    try {
      ...
      perfMonitor.end('initializePurchasePage');
  ...
  fetchPurchases: async (forceRefresh = false) => {
    perfMonitor.start('fetchPurchases');
    ...
    if (!forceRefresh) {
      ...
      if (cached) {
        ...
        perfMonitor.end('fetchPurchases');
        return;
      }
    }
  ...
  returnPurchase: async (returnData, refundChoice) => {
    perfMonitor.start('returnPurchase');
    set({ isLoading: true });
    ...
    try {
      ...
      set({ isLoading: false });
      perfMonitor.end('returnPurchase');
